...
​	char、byte、short等类型进行移位操作的时候会默认先转为int类型。得到的结果也是一个int类型的数值。此时**右操作数只有低五位才有用**，因为int只有32位，2的五次方是32。同理，对long类型进行移位操作的时候，右操作数只有低六位是有效的，得到的也是一个long类型的结果。
...
​	对于char、byte、short，复合赋值（指的是使用+=、-=、>>>=等符号赋值）不需要进行类型转换，尽管有类型的提升，但是其结果和直接运算的结果相同，而且不使用类型转换也可以使代码更加简练。

​	除布尔类型之外，任何一种基本类型都可以通过类型转换变为其它类型。

## 3.18 总结

​	本章主要是操作符的基础知识，对于有一定经验的读者来说，需要注意的只是以前没有注意的细节。
...
​	[Answer](Exercises/Code/02/Nickname.java)

### 练习3

​	创建一个包含一个float域的类，并用这个类来展示方法调用时的别名机制。

​	[Answer](Exercises/Code/03/Nickname.java)
...
​	[Answer](Exercises/Code/06/Equals.java)

### 练习7

​	编写一个程序，模拟扔硬币的结果。

​	[Answer](Exercises/Code/07/Coin.java)
